...
I_max_array=[1,2,3,4,5];
L_I_max_array=length(I_max_array);
v=2000;%振荡器线宽,单位Hz
Nt=2;%接收天线数
Nr=64;%发射天线数
Nrf=2;%射频链数
...
for kkk=1:Nit1
    ...
    y_store=y;
    %% 不同信噪比开始
    for iii=1:L_I_max_array%每次迭代使用不同信噪比
        iii
        %% 接收信号
        %生成高斯白噪声向量
        power_w=1;
        w=sqrt(power_w/2)*(randn(Nrf*N*ofdm_num,1)+sqrt(-1)*randn(Nrf*N*ofdm_num,1));%生成高斯白噪声
        y=y_store*sqrt(rhos)+w;
        %% prop
        adjust_num=I_max_array(iii);%最大调整次数
            ...
            for j5_transmit=1:L
                ...
                for j1=1:adjust_pn_num%共调整五个值（adjust_pn_num=5）
                    for j3=1:2%分实部虚部
                        ...
                        if j1==1||j1==4
                            if j3==1
                                step_length=twoStepR;
                            end
                            if j3==2
                                step_length=twoStepI;
                            end
                        end
                        if j1==2||j1==3
                            if j3==1
                                step_length=threeStepR;
                            end
                            if j3==2
                                step_length=threeStepI;
                            end
                        end
                        if j1==5
                            if j3==1
                                step_length=oneStepR;
                            end
                            if j3==2
                                step_length=oneStepI;
                            end
                        end
                        %% 判断调整第几个值
                        if j1==1%调整a[2]
...
figure
hold on
plot(I_max_array,mean_array_error_prop4_7_4_db,'r-o','Markersize',8,'Linewidth',1.5)
plot(I_max_array,mean_array_error_method5_db,'b-+','Markersize',8,'Linewidth',1.5)
plot(I_max_array,mean_array_error_method6_db,'k-V','Markersize',8,'Linewidth',1.5)
plot(I_max_array,mean_LS_error_h_upper_db,'y-V','Markersize',8,'Linewidth',1.5)
plot(I_max_array,mean_LS_error_h_lower_db,'g-+','Markersize',8,'Linewidth',1.5)
xlabel('SNR(dB)');
ylabel('NMSE(dB)');
...
hold off
%% 数据储存dB
data_store1=zeros(L_I_max_array,5);%共5条线
