...
v=2000;%振荡器线宽,单位Hz
L_array=[1,2,3,4];%信道抽头
% L_array=[2];%信道抽头
L_length=length(L_array);
Nt=2;%接收天线数
Nr=64;%发射天线数
Nrf=2;%射频链数
Nt_beam=Nt;%假设的值
...
for kkk=1:Nit1
    ...
    %% 不同时域信道长度开始
    for iii=1:L_length%每次迭代使用不同时域信道长度
        iii
        L=L_array(iii);
        ofdm_num=L;
        Q=zeros(Nrf*N*L,M*M*L);%清空Q
        Q_LS=zeros(Nrf*N*L,Nr*Nt*L);%清空Q_LS
        %% 确定Q的秩对应的K
        if L==1
            K_rank=128;
        end
        ...
        for j4_transmit=1:adjust_num
            ...
            for j5_transmit=1:L
                ...
                for ofdmi=1:ofdm_num%获得总的Q
                    ...
                    Q((ofdmi-1)*Nrf*N+1:ofdmi*Nrf*N,1:M*M*L)=sub_Q;
                end
        ...
        for ofdmi=1:ofdm_num%获得总的Q
            ...
            Q_LS((ofdmi-1)*Nrf*N+1:ofdmi*Nrf*N,1:Nr*Nt*L)=sub_Q;
        end
...
figure
hold on
plot(L_array,mean_array_error_prop4_7_4_db,'r-o','Markersize',8,'Linewidth',1.5)
plot(L_array,mean_array_error_method5_db,'b-+','Markersize',8,'Linewidth',1.5)
plot(L_array,mean_array_error_method6_db,'k-V','Markersize',8,'Linewidth',1.5)
plot(L_array,mean_LS_error_h_upper_db,'y-V','Markersize',8,'Linewidth',1.5)
plot(L_array,mean_LS_error_h_lower_db,'g-+','Markersize',8,'Linewidth',1.5)
xlabel('Time domain channel length (L)');
ylabel('NMSE(dB)');
...
hold off
%% 数据储存dB
data_store1=zeros(L_length,5);%共5条线
data_store1(:,1)=L_array.';
...
figure 
hold on 
plot(array_error_prop4_7_4(:,1),'r-o','Markersize',8,'Linewidth',1.5);
plot(array_error_without_adjust_prop4_7_4(:,1),'b-V','Markersize',8,'Linewidth',1.5);
legend('prop','不调整');
hold off
