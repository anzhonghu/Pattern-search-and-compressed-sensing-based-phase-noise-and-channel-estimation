...
for kkk=1:Nit1
    ...
    for iii=1:L_M%每次迭代使用不同信噪比
        iii
        M=M_array(iii);%把0-2pi角度划分成M个
        Q=zeros(Nrf*N, M*M*L);
        %% 生成AR和AT
        AR=zeros(Nr,M);
        for i=1:M
...
figure
hold on
plot(M_array,mean_array_error_prop4_7_4_db,'r-o','Markersize',8,'Linewidth',1.5)
plot(M_array,mean_array_error_method5_db,'b-+','Markersize',8,'Linewidth',1.5)
plot(M_array,mean_array_error_method6_db,'k-V','Markersize',8,'Linewidth',1.5)
plot(M_array,mean_LS_error_h_upper_db,'y-V','Markersize',8,'Linewidth',1.5)
plot(M_array,mean_LS_error_h_lower_db,'g-+','Markersize',8,'Linewidth',1.5)
xlabel('SNR(dB)');
ylabel('NMSE(dB)');
